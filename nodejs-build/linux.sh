...
mkdir -p ../puerts-node/nodejs/lib/Linux/[ -z "$GITHUB_WORKSPACE" ] && GITHUB_WORKSPACE="$( cd "$( dirname "$0" )"/.. && pwd )"
...
ls -l out/Release/
cp out/Release/libnode.so.* ../puerts-node/nodejs/lib/Linux/libnode.so