(function getInternalJSLib(internalNativeLib) {
    var global = this;
    ...
    global.__puer_path__ = {
        normalize: normalize,
        isAbsolute(filepath) {
            // do not fix the RegExp here. It will replace in .h by CMake
            return !(
                !/^[\\\\\/]{2,}[^\\\\\\/]+[\\\\\\/]+[^\\\\\\/]+/.test(filepath) &&
                !/^([a-z]:)?[\\\\\\/]/i.test(filepath)
            )
    ...
    // we should return getJSObject at first.
    // and C# can use it to get other Internal Function.
    const getJSObjectValue = function getJSObjectValue(jsobject, key) {
        return jsobject[key];
    }

    Object.assign(getJSObjectValue, {
        pathNormalize: normalize,
        pathIsAbsolute: __puer_path__.isAbsolute,
        pathIsRelative: __puer_path__.isRelative,
        pathDirname: __puer_path__.dirname,

        addSyntheticModule(moduleName, obj) {
            if (typeof obj != 'object') throw new Error('addModule: the module namespace should be a object');
            const keys = Object.keys(obj);
            internalNativeLib.CreateSyntheticModule(moduleName, keys, obj);
        },
        getModuleExecutor() {
            var loader = typeof jsEnv != 'undefined' ? jsEnv.GetLoader() : __tgjsGetLoader();
            global.__puer_js_resolve_module_url__ = function (specifier, referer) {
                ...
                if (!loader.Resolve) {
                    ...
                    if (internalNativeLib.HasSyntheticModule && internalNativeLib.HasSyntheticModule(s)) {
                        return s
                    } else if (loader.FileExists(s)) {
                        return s
                    } else {
            ...
            global.__puer_js_resolve_module_content__ = function (specifier, debugpathRef = []) {
                const originSp = specifier;
            ...
            return internalNativeLib.ModuleExecutor;
        }
    });
