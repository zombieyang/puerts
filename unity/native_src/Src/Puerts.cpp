    ...
    else
    {
        if (Value->IsArrayBufferView())
        {
            ...
            *Length = static_cast<int>(BuffView->ByteLength());
            auto ABBS = BuffView->Buffer()->GetBackingStore().get();
            return static_cast<char*>(ABBS->Data()) + BuffView->ByteOffset();
        }
        else if (Value->IsArrayBuffer())
        {
            auto Ab = v8::ArrayBuffer::Cast(Value);
            auto ABBS = Ab->GetBackingStore().get();
            *Length = static_cast<int>(ABBS->ByteLength());
            return static_cast<char*>(ABBS->Data());
        }
        else
        {
            return nullptr;
