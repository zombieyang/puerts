                ...
                if (!isDelegate)
                {
                    if (ctors != null && ctors.Length > 0 && (!type.IsArray || type == typeof(System.Array)))
                    {
                        foreach (var ctor in ctors)
                        {
                            ...
                            {
                                if (!pis[i].HasDefaultValue) break;
                                ...
                                var mi = DVAdaptorType.GetMethod("ctor", types.ToArray());
                                if (mi == null) UnityEngine.Debug.Log(ctor.DeclaringType + ".ctor(" + String.Join(",", types.Select(type=> type.FullName)) + ") is null");
                                AddConstructorToType(ctor as ConstructorInfo, newPis.ToArray(), mi);
                            }
                        }
                    }
                    ...
                    if (methods != null && (!type.IsArray || type == typeof(System.Array)))
                    {
                        foreach (var method in methods)
                        {
                            ...
                            for (var i = pis.Length - 1; i >= 0; i--)
                            {
                                ...
                                if (mi == null) UnityEngine.Debug.Log(method.DeclaringType + "." + method.Name + "(" + String.Join(",", types.Select(type=> type.FullName)) + ") is null");
